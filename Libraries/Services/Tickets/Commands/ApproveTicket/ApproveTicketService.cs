...
using Helpdesk.Services.Tickets.Events.ApproveTicket;
...
{
    public class ApproveTicketService : IApproveTicketService
    {
        private readonly IContextRepository<ITicketContext> _repository;
        private readonly INotificationService _notificationService;
        private readonly IWorkflowService _workflowService;

        public ApproveTicketService(
        ...
        public virtual async Task<ProcessResult> Approve(int ticketId, int userId)
        {
            ...
            switch (ticket.GetStatus())
            {
                ...
                case TicketStatus.Approved:
                    return new TicketAlreadyApprovedResult(ticketId, ticket.ApprovedBy.Value);
            }
            ...
            await _workflowService.Process(new BeforeTicketApprovedWorkflow(ticketId, userId));

            ticket.Approve(userId);
            await _repository.SaveAsync();

            var workflow = _workflowService.Process(new Events.RequestApproval.TicketApprovalRequestWorkflow(ticketId, userId));
