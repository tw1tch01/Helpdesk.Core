...
namespace Helpdesk.Services.UnitTests.Tickets.Commands
{
    ...
    public class DeleteTicketServiceTests
    {
        ...
        public async Task Delete_WhenTicketRecordIsNull_VerifyFactoryTicketNotFoundIsReturned()
        {
            ...
            mockRepository.Verify(v => v.SingleAsync(It.Is<GetTicketById>(t => t._ticketId == ticketId)), Times.Once, "Should call the context's SingleAsync exactly once for GetTicketById.");
        }
        ...
        public async Task Delete_BeforeTicketIsDeleted_VerifyBeforeTicketDeletedWorkflowIsProcessed()
        {
            ...
            mockWorkflowService.Verify(v => v.Process(It.IsAny<BeforeTicketDeletedWorkflow>()), Times.Once, "Should call the workflow services's Process method for BeforeTicketDeletedWorkflow.");
        }
        ...
        public async Task Delete_WhenBeforeTicketDeletedWorkflowIsNotSuccessful_VerifyFactoryWorkflowFailedIsReturned()
        {
            ...
            var mockBeforeTickedDeletedWorkflow = new Mock<BeforeTicketDeletedWorkflow>(ticketId, userId);
            var mockRepository = new Mock<IContextRepository<ITicketContext>>();
            var mockWorkflowService = new Mock<IWorkflowService>();
            var mockFactory = new Mock<IDeleteTicketResultFactory>();

            mockBeforeTickedDeletedWorkflow.Setup(a => a.Result).Returns(WorkflowResult.Failed);
            mockRepository.Setup(s => s.SingleAsync(It.IsAny<GetTicketById>())).ReturnsAsync(new Ticket());
            mockWorkflowService.Setup(s => s.Process(It.IsAny<BeforeTicketDeletedWorkflow>())).ReturnsAsync(mockBeforeTickedDeletedWorkflow.Object);
            ...
            mockFactory.Verify(v => v.WorkflowFailed(ticketId, userId, mockBeforeTickedDeletedWorkflow.Object), Times.Once, "Should return the factory's WorkflowFailed method.");
        }
