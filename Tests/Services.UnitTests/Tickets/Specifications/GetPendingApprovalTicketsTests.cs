...
{
    [TestFixture]
    public class GetPendingApprovalTicketsTests
    {
        [Test]
        public void IsSatisfiedBy_WhenTicketIsPendingApproval_ReturnsTrue()
        {
            var ticket = CreateTicket(approvalRequestedOn: DateTimeOffset.UtcNow);
            var spec = new GetPendingApprovalTickets();
            var result = spec.IsSatisfiedBy(ticket);
            Assert.IsTrue(result, "Should return true when status is PendingApproval.");
        }
