...
{
    [TestFixture]
    public class GetApprovedTicketsTests
    {
        [Test]
        public void IsSatisfiedBy_WhenTicketIsApproved_ReturnsTrue()
        {
            var ticket = CreateTicket(approvedOn: DateTimeOffset.UtcNow);
            var spec = new GetApprovedTickets();
            var result = spec.IsSatisfiedBy(ticket);
            Assert.IsTrue(result, "Should return true when status is Approved.");
        }
